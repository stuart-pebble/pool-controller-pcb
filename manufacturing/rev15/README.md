...
- `BOM.csv`: 19 carrier components (15 SMT and 4 through-hole). J1/J2 specify Ckmtw R-RJ11R06P-A000 / LCSC C2902699. All carrier parts have nominated supplier links. Legacy capacitors, L1 and the obsolete J4 socket need stock/lifecycle confirmation.
- `PCBWay-BOM.csv`: use for the carrier quote; includes all 19 carrier components. Waveshare boards are already customer-owned and must not be purchased again.
- `PCBWay-SMT-CPL.csv`: use as PCBWay’s SMT centroid file (15 SMT parts). The four through-hole components and module fitting are covered by the BOM/drawing/instructions.
